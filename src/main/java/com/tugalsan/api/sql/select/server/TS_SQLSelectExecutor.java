...
public class TS_SQLSelectExecutor {
    ...
    public void walk(TGS_CallableType1_Run<TS_SQLResultSet> onEmpty, TGS_CallableType1_Run<TS_SQLResultSet> rs) {
    ...
    public void walkRows(TGS_CallableType1_Run<TS_SQLResultSet> onEmpty, TGS_CallableType2_Run<TS_SQLResultSet, Integer> rs_ri) {
        walk(onEmpty, rs -> rs.walkRows(null, ri -> rs_ri.run(rs, ri)));
    }

    public void walkCells(TGS_CallableType1_Run<TS_SQLResultSet> onEmpty, TGS_CallableType3_Run<TS_SQLResultSet, Integer, Integer> rs_ri_ci) {
        walk(onEmpty, rs -> rs.walkCells(null, (ri, ci) -> rs_ri_ci.run(rs, ri, ci)));
    }

    public void walkCols(TGS_CallableType1_Run<TS_SQLResultSet> onEmpty, TGS_CallableType2_Run<TS_SQLResultSet, Integer> rs_ci) {
